...
-- GET MPRs
GO
CREATE PROC GET_MPR_BY_PROJECT @PROJECT_ID UNIQUEIDENTIFIER, @IS_CANCEL BIT
AS
BEGIN
	SET NOCOUNT ON;
	SELECT 
	...
	AND m.IS_CANCEL = @IS_CANCEL
END
GO	

-- GET MPRs CANCEL
GO
CREATE PROC GET_MPR_CANCEL_BY_PROJECT @PROJECT_ID UNIQUEIDENTIFIER, @IS_CANCEL BIT
AS
BEGIN
	SET NOCOUNT ON;
	SELECT 
...
-- GET MPRs for Add Create POs
GO
CREATE PROC GET_MPR_FOR_CREATE_PO_BY_PROJECTS @PROJECT_ID UNIQUEIDENTIFIER
AS
BEGIN
	SET NOCOUNT ON;
	SELECT 
...
EXEC GET_MPR_FOR_CREATE_PO_BY_PROJECTS '3AF405BC-304C-4847-A7E0-07288AD3B114'
-- Get MPR detail
GO
...
-- Get PO V2
GO
CREATE PROC GET_POS_V2 @PROJET_ID UNIQUEIDENTIFIER
AS
BEGIN
	SELECT POS.ID, PO_NO, PO_MPR_NO, PO_WO_NO, PO_PROJECT_NAME, PO_REV_TOTAL, PO_CREATE_DATE, PO_EXPECTED_DELIVERY_DATE, PO_PREPARED,
		PO_REVIEWED, PO_AGREEMENT, PO_APPROVED, PO_PAYMENT_TERM, PO_TOTAL_AMOUNT,
		SUPPLIERS.NAME, POS.SUPPLIER_ID
	FROM POS, SUPPLIERS
	WHERE POS.SUPPLIER_ID = SUPPLIERS.ID AND POS.PROJECT_ID = @PROJET_ID
END
GO
...
-- Get PO for Import product V2
GO
CREATE PROC GET_POS_FOR_IMPORT_PRODUCT_V2 @PROJECT_ID UNIQUEIDENTIFIER
AS
BEGIN
	SELECT POS.ID, PO_NO, PO_MPR_NO, PO_WO_NO, PO_PROJECT_NAME, PO_REV_TOTAL, PO_CREATE_DATE, PO_EXPECTED_DELIVERY_DATE, PO_PREPARED,
		PO_REVIEWED, PO_AGREEMENT, PO_APPROVED, PO_PAYMENT_TERM, PO_TOTAL_AMOUNT, SUPPLIERS.NAME
	FROM POS, SUPPLIERS
	WHERE POS.SUPPLIER_ID = SUPPLIERS.ID AND POS.IS_IMPORTED = 0 AND POS.PROJECT_ID = @PROJECT_ID
END
GO
...
-- GET Import V2
GO
CREATE PROC GET_IMPORTS_V2 @PROJECT_ID UNIQUEIDENTIFIER
AS
BEGIN
	SELECT IMPORT_PRODUCTS.ID, FROM_PO_NO, IMPORT_DATE, STAFF_NAME
	FROM IMPORT_PRODUCTS, STAFFS
	WHERE IMPORT_PRODUCTS.STAFF_ID = STAFFS.ID AND IMPORT_PRODUCTS.PROJECT_ID = @PROJECT_ID
END
GO
...
AS
BEGIN
	SELECT IMPORT_PRODUCT_DETAIL.ID, IMPORT_PRODUCT_ID, IMPORT_PRODUCT_DETAIL.PRODUCT_ID, PRODUCTS.PRODUCT_NAME,
...
GO
  ...
  EXEC GET_IMPORT_DETAIL_BY_ID '341C2785-1670-4FA1-903D-4A34D5BF9C64'
...
-- GET WAREHOUSE DETAIL BY WAREHOUSE ID
GO
CREATE PROC GET_WAREHOUSE_DETAILBY_ID @WAREHOUSE_ID UNIQUEIDENTIFIER
AS
BEGIN
...
-- GET MATERIAL TYPE
GO
CREATE PROC GET_MATERIAL_TYPE
AS
BEGIN
	SELECT ID, MATERIAL_TYPE_CODE, MATERIAL_TYPE_NAME
	FROM MATERIAL_TYPE_DETAIL
	ORDER BY MATERIAL_TYPE_CODE
END
GO

-- GET MATERIAL TYPE FOR COMBOBOX
GO
CREATE PROC GET_MATERIAL_TYPE_FOR_COMBOX
AS
BEGIN
	SELECT *
	FROM MATERIAL_TYPE_DETAIL
...
-- GET MATERIAL TYPE
GO
CREATE PROC GET_MATERIAL_TYPE_BY_TYPE @TYPE_ID UNIQUEIDENTIFIER
AS
BEGIN
	SELECT ID, MATERIAL_TYPE_CODE, MATERIAL_TYPE_NAME
	FROM MATERIAL_TYPE_DETAIL
	WHERE MATERIAL_TYPE_DETAIL.MATERIAL_TYPES_ID = @TYPE_ID
	ORDER BY MATERIAL_TYPE_CODE 
END
GO
...
---- GET LIST ITEM OF MATERIAL TYPE
GO
...
GO
CREATE PROCEDURE GET_STAFF_MANAGER
AS
BEGIN
	SELECT ID, STAFF_NAME
	FROM STAFFS
	WHERE STAFF_ROLE_ID = 'ba3dee51-feb6-4586-a1ad-8a62567eae02'
END
GO